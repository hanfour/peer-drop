...
final class SpriteCacheTests: XCTestCase {
    ...
    func testCacheStoreAndRetrieve() {
        ...
        let key = SpriteCache.Key(body: .cat, stage: .baby, action: .idle, frame: 0, paletteIndex: 0, scale: 1)
    ...
    func testCacheEvictsOldEntries() {
        ...
        let img = TestSpriteHelper.make(width: 16, height: 16, fillIndex: 1)
        let k1 = SpriteCache.Key(body: .cat, stage: .baby, action: .idle, frame: 0, paletteIndex: 0, scale: 1)
        let k2 = SpriteCache.Key(body: .cat, stage: .baby, action: .idle, frame: 1, paletteIndex: 0, scale: 1)
        let k3 = SpriteCache.Key(body: .cat, stage: .baby, action: .walking, frame: 0, paletteIndex: 0, scale: 1)
