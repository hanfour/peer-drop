...
final class SpriteServiceAnimationTests: XCTestCase {
    ...
    // MARK: - C1 regression: v3 zip lacking the requested direction

    func test_decode_v3PartialZip_walkSouthPresent_returnsAnimatedFrames() async throws {
        let service = SpriteService(bundle: testBundle)
        let request = AnimationRequest(
            species: SpeciesID("test-partial"),
            stage: .baby,
            direction: .south,
            action: .walking
        )
        ...
        let frames = try await service.framesInternal(
            at: url(forFixture: "test-anim-v3-partial"),
            for: request
        )

        XCTAssertEqual(frames.images.count, 8, "south has the full walk animation")
        XCTAssertEqual(frames.fps, 6)
    }

    func test_decode_v3PartialZip_walkEastMissing_fallsBackToRotationSingleFrame() async throws {
        // Production cat-tabby-adult.zip ships partial coverage (walk/south
        // only). Asking the renderer for east while walking must NOT throw —
        // it should degrade gracefully to the east rotation PNG as a 1-frame
        // static. Without this fallback, PetEngine.updateRenderedImage's
        // try? swallows the throw and the pet vanishes the moment it changes
        // direction. This regression test pins the contract.
        let service = SpriteService(bundle: testBundle)
        let request = AnimationRequest(
            species: SpeciesID("test-partial"),
            stage: .baby,
        ...
        XCTAssertEqual(frames.images.count, 1,
                       "missing v3 direction falls back to 1-frame static rotation")
        XCTAssertEqual(frames.fps, 1, "fallback fps=1 so animator timer doesn't churn")
        XCTAssertFalse(frames.loops)
    }

    func test_decode_v3PartialZip_idleAnyDirection_fallsBackToRotation() async throws {
        // No idle animation in the partial zip at all — should degrade like v2.
        let service = SpriteService(bundle: testBundle)
        let request = AnimationRequest(
            species: SpeciesID("test-partial"),
            stage: .baby,
