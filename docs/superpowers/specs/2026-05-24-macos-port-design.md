...
- **M1d — File migration into modules** (split into 4 sub-PRs, 2026-05-26). Real scope: **175 source files** (not 90 — Discovery, Voice transport-side, Telemetry, Pet renderer all undercounted) + **324 Pet resource zips** + **~87 test files** needing `@testable import` updates + 3 reviewer follow-ups from M1c (CallKitManager path placement, `swift build` CI gate, per-module `Tests/` targets). Each module migration is atomic — `internal` access breaks across module boundaries, so files move in groups. Restructured as four PRs that each ship independently:
  ...
  - **M1d-3a — PeerDropPlatform extraction** (split further during M1d-3 planning 2026-05-26): Extract `PeerDrop/Core/Platform/` (17 files: registry + 7 protocols + 7 iOS adapters + 2 helpers) + `PeerDrop/Core/HapticManager.swift` (1 file, facade) into a NEW `PeerDropPlatform` module — a 6th product in PeerDropKit, more foundational than the 4 leaves. Move 5 test files into PeerDropPlatformTests. Update ~10 consumer files across Pet/Transport/UI/Core to `import PeerDropPlatform`. NO Pet/Core file migration — just the Platform/ subdir + HapticManager. ~1.5 days, ~7 tasks.
  - **M1d-3b — Pet migration**: 61 source files (preserving 8 subdirs) + 63 tests + 324 resource zips. Pet depends on PeerDropPlatform (no cycle). Bundle.main → Bundle.module (4 sites). Widget rewires from 8 path-references to `- package: PeerDropPet`. ~2.5 days, ~10 tasks.
  - **M1d-4 — Transport + Voice migration**: PeerDropTransport (28 files: Transport 16 + Discovery 6 + Voice transport-side 6). CallKitManager stays in app target. Likely depends on PeerDropPlatform (TBD until Transport audit). ~1-2 days.
  - **M1d-5 — Core migration + final cleanup**: PeerDropCore (now ~28 files — Platform/ + HapticManager already extracted in M1d-3a). Remove now-unused direct WebRTC/ZIPFoundation deps from app target. Final lint validation. PeerIdentity placement decision. ~2 days.
