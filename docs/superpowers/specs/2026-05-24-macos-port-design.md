...
- **M1d — File migration into modules** (split into 4 sub-PRs, 2026-05-26). Real scope: **175 source files** (not 90 — Discovery, Voice transport-side, Telemetry, Pet renderer all undercounted) + **324 Pet resource zips** + **~87 test files** needing `@testable import` updates + 3 reviewer follow-ups from M1c (CallKitManager path placement, `swift build` CI gate, per-module `Tests/` targets). Each module migration is atomic — `internal` access breaks across module boundaries, so files move in groups. Restructured as four PRs that each ship independently:
  ...
  - **M1d-2 — Protocol + Security leaves** (split further during execution 2026-05-26 when Pet audit surfaced a Platform-helper cycle): PeerDropProtocol (11 files), PeerDropSecurity (28 files incl. Security/Protocol/ subdir + 22 test files + CryptoTestKit). Pet deferred. PeerDropProtocol stays pure via factory extraction (4 PeerMessage factories → `PeerMessage+Hello.swift` in Core). PeerDropSecurity stays pure via protocol-inversion bridges (`RelayAuthDeviceStore`, `MailboxServiceProtocol`). ~2 days, 6 tasks.
  - **M1d-3 — Platform extraction + Pet migration**: PetRendererV3 + PetEngine need `PlatformImage` / `PlatformGraphicsRenderer` / `HapticManager` — currently in `PeerDrop/Core/Platform/`. Per spec §1, Core depends on Pet (leaf); Pet → Core would cycle. Resolution: extract `PeerDrop/Core/Platform/` (~18 files: registry + 7 protocols + 7 iOS adapters + 3 typealiases/helpers + HapticManager) into a NEW `PeerDropPlatform` module — a 6th product in PeerDropKit, MORE foundational than the 4 leaves. PeerDropPet → PeerDropPlatform. Then migrate Pet (61 source + 324 resources + 63 tests). Widget rewires from path-refs to `- package: PeerDropPet`. ~3-4 days.
  - **M1d-4 — Transport + Voice migration**: PeerDropTransport (28 files: Transport 16 + Discovery 6 + Voice transport-side 6). CallKitManager stays in app target. May also need PeerDropPlatform (TBD until Transport audit). ~1-2 days.
  - **M1d-5 — Core migration + final cleanup**: PeerDropCore (now smaller — ~29 files after Platform/ extracted in M1d-3). Remove now-unused direct WebRTC/ZIPFoundation deps from app target. Final lint validation. ~2 days.

Total realistic M1: **~14-18 working days, ~60+ tasks across 8 PRs (M1a/b/c/d-1/d-2/d-3/d-4/d-5)**. Original "5–7 days, 1 PR" estimate was wildly optimistic (~3x off). Third underestimation surfaced 2026-05-26: Pet → Platform helpers cycle forces a PeerDropPlatform extraction (Spec §1's 4-leaf graph becomes 5 modules: PeerDropPlatform as a sub-leaf below the existing 4).
