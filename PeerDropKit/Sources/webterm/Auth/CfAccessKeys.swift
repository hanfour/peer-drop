...
public enum CfAccessKeys {
    ...
    public static func certsURL(team: String) -> URL {
        URL(string: "https://\(team).cloudflareaccess.com/cdn-cgi/access/certs")!
    }
    ...
    /// - Throws: `CfAccessKeysError.fetchFailed` on network/HTTP errors; JSON
    ...
    public static func fetch(team: String) async throws -> JWTKeyCollection {
        let url = certsURL(team: team)
        let (data, response) = try await URLSession.shared.data(from: url)
        guard let http = response as? HTTPURLResponse,
              (200..<300).contains(http.statusCode) else {
            throw CfAccessKeysError.fetchFailed
        }
...
public enum CfAccessKeysError: Error {
    case fetchFailed
}
