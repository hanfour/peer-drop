...
@MainActor
public final class NearbyInteractionManager: NSObject, ObservableObject {

    /// Published proximity data keyed by peer ID.
    @Published public private(set) var peerProximity: [String: ProximityInfo] = [:]

    public struct ProximityInfo {
        public var distance: Float?      // metres
        public var direction: SIMD3<Float>?

        public init(distance: Float? = nil, direction: SIMD3<Float>? = nil) {
    ...
    public func startSession(for peerID: String, sendToken: @escaping (Data) -> Void) {
    ...
    public func handleTokenOffer(_ tokenData: Data, from peerID: String, respond: @escaping (Data) -> Void) {
    ...
    public func handleTokenResponse(_ tokenData: Data, from peerID: String) {
        guard let peerToken = try? NSKeyedUnarchiver.unarchivedObject(ofClass: NIDiscoveryToken.self, from: tokenData) else {
    ...
    public func stopSession(for peerID: String) {
...
extension NearbyInteractionManager: NISessionDelegate {
    public nonisolated func session(_ session: NISession, didUpdate nearbyObjects: [NINearbyObject]) {
    ...
    public nonisolated func session(_ session: NISession, didRemove nearbyObjects: [NINearbyObject], reason: NINearbyObject.RemovalReason) {
        Task { @MainActor [weak self] in
            guard let self, let peerID = self.peerID(for: session) else { return }
            switch reason {
            case .peerEnded:
