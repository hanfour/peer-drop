...
public final class VoiceCallManager: ObservableObject {
    ...
    public func session(for peerID: String) -> VoiceCallSession {
    ...
    public func startCall(to peerID: String) async {
        guard let host = host else { return }
    ...
    public func handleCallRequest(from senderID: String) {
        // Determine peer name
        let peerName: String
    ...
    public func handleCallReject(reason: String? = nil) {
        callProvider.reportCallEnded(reason: .declinedElsewhere)
        endCallLocally()
    }

    public func handleCallEnd() {
    ...
    public func handleSignaling(_ message: PeerMessage) {
