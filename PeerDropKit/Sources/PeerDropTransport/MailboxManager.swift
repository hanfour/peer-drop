...
public final class MailboxManager: ObservableObject {
    ...
    @Published public private(set) var mailboxId: String?
    @Published public private(set) var isRegistered = false
    @Published public private(set) var pendingMessages: [MailboxMessage] = []
    ...
    public var onMessageReceived: ((MailboxMessage) -> Void)?

    public init(client: MailboxClient = MailboxClient(), preKeyStore: PreKeyStore = PreKeyStore()) {
        self.client = client
    ...
    public func pollMessages() async {
    ...
    public func uploadPreKeysIfNeeded() async {
    ...
    public func rotateMailbox() async throws {
