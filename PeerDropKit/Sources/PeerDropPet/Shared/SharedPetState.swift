...
public struct PetSnapshot: Codable, Equatable {
    ...
    public init(name: String?, bodyType: BodyGene, eyeType: EyeGene, patternType: PatternGene, level: PetLevel, mood: PetMood, paletteIndex: Int, evolutionProgress: Double) {
        self.name = name; self.bodyType = bodyType; self.eyeType = eyeType; self.patternType = patternType
        self.level = level; self.mood = mood; self.paletteIndex = paletteIndex; self.evolutionProgress = evolutionProgress
    }
}
...
public class SharedPetState {
    ...
    public func write(_ snapshot: PetSnapshot) {
        let data: Data
        do {
