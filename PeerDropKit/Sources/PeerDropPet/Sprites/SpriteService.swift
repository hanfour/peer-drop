...
public enum SpriteServiceError: Error {
...
public actor SpriteService {

    /// Shared singleton. Uses Bundle.module (the PeerDropPet resource bundle)
    /// so assets are resolved from the SPM package after Task 7 moves the
    /// Pets/ zips into PeerDropPet/Resources/. Bundle.module is internal
    /// so it can't appear as a default argument value in a public init —
    /// the shared instance captures it explicitly here.
    public static let shared = SpriteService(cache: .shared, bundle: .module)
    ...
    public func image(for request: SpriteRequest) async throws -> CGImage {
    ...
    public func frames(for request: AnimationRequest) async throws -> AnimationFrames {
    ...
    public func framesInternal(at zipURL: URL, for request: AnimationRequest) async throws -> AnimationFrames {
        return try await loadFrames(at: zipURL, for: request)
    }
    #endif
