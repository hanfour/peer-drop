...
@MainActor
public class PetEngine: ObservableObject {
    @Published public var pet: PetState {
        didSet {
    ...
    @Published public var currentDialogue: String?
    @Published public private(set) var renderedImage: CGImage?
    @Published public var physicsState: PetPhysicsState = PetPhysicsState(
        position: CGPoint(x: 60, y: 200), velocity: .zero, surface: .ground)
    @Published public var particles: [PetParticle] = []
    @Published public var poopState = PoopState()
    @Published public var showEvolutionFlash = false

    public struct DroppedFood {
        public let type: FoodType
        public let position: CGPoint
        public init(type: FoodType, position: CGPoint) { self.type = type; self.position = position }
    }

    @Published public var foodTarget: DroppedFood?
    ...
    public var evolutionProgress: Double {
    ...
    public var currentLifeState: PetLifeState {
    ...
    public func handleInteraction(_ type: InteractionType) {
        let now = Date()
    ...
    public func handleChatMessage() {
        handleInteraction(.chatActive)
        triggerChatBehavior()
    }

    public func reactionForEvent(_ event: InteractionType) -> PetAction {
        pet.genome.personalityTraits.reaction(to: event)
    }

    // MARK: - Poop & Stroke

    public func cleanPoop(id: UUID) {
    ...
    @discardableResult
    public func migrateAgingForV5() -> Bool {
    ...
    @discardableResult
    public func migrateGhostBodyForV501() -> Bool {
        do {
    ...
    // MARK: - Rendering
    public func updateRenderedImage() {
    ...
    public static let walkEnterThreshold: Double = 5.0
    ...
    public static let walkExitThreshold: Double = 3.0
    ...
    public static func nextAction(previous: PetAction, velocity: CGVector) -> PetAction {
    ...
    public static func actionFromVelocity(_ velocity: CGVector) -> PetAction {
...
extension PersonalityTraits {
    public func reaction(to event: InteractionType) -> PetAction {
        switch event {
        case .tap:
...
public enum PetTimeOfDayBehavior {
    public static func suggestedMood(at date: Date = Date(), lastInteraction: Date) -> PetMood? {
