...
public class InteractionTracker {
    public struct Record: Codable {
        ...
        public init(type: InteractionType, date: Date) { self.type = type; self.date = date }
    }

    public private(set) var allHistory: [Record] = []

    public var recentHistory: [Record] {
        let cutoff = Date().addingTimeInterval(-86400)
        return allHistory.filter { $0.date > cutoff }
    }

    public var lastHourHistory: [Record] {
        let cutoff = Date().addingTimeInterval(-3600)
        return allHistory.filter { $0.date > cutoff }
    }

    /// Total tap XP earned today
    public var tapXPToday: Int {
    ...
    public func record(_ type: InteractionType) {
        allHistory.append(Record(type: type, date: Date()))
        trimOldHistory()
    }

    public func calculateMood(hasSocialRecently: Bool) -> PetMood {
    ...
    // For testing only
    public func insertForTesting(_ record: Record) {
