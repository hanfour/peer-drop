...
public enum SpriteSheetLoader {
    ...
    public static func slice(strip: CGImage, frameSize: Int = Self.frameSize) throws -> [CGImage] {
    ...
    /// Read the color indices from a CGImage into a 2D array.
    /// Each pixel's red channel value is treated as the palette index (0-15).
    public static func readIndices(from image: CGImage) -> [[UInt8]] {
        let w = image.width
        let h = image.height
...
extension CGImage {
    public static func from(pngData data: Data) -> CGImage? {
